...
The site behaves like live infrastructure, not static pages:

- Animated traffic topology in the hero (`src/containers/greeting/TopologyHero.js`)
- journalctl-style event ticker (`src/containers/journalTicker/`)
- tmux-style numbered nav tabs with a ticking clock and status pulse
- Command palette on Ctrl+K (`src/components/commandPalette/`), which can run
  terminal commands from anywhere
- htop-style role summary on the experience page (`src/containers/htopExperience/`)
- Route changes animate as pane wipes (`src/containers/Main.css`)
- Keyboard shortcuts: `0–4` switch pages, `j/k` scroll

Self-hosted fonts live in `public/fonts/` (IBM Plex Sans + JetBrains Mono).
The social card is generated by `python3 scripts/generate_og.py` into
`public/icons/og-card.png`.
